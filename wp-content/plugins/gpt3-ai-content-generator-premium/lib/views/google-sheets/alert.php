...
if(!empty($wpaicg_crojob_sheets_last_time)){
    $wpaicg_timestamp_diff = time() - $wpaicg_crojob_sheets_last_time;
    if($wpaicg_timestamp_diff > 600){
        ...
        <div class="wpaicg-alert">
            ...
            <form action="" method="post">
                ...
                <button name="wpaicg_delete_sheets_running" class="button button-primary"><?php echo esc_html__('Force Refresh','gpt3-ai-content-generator')?></button>
            </form>
        </div>
        <?php
    }
...
<div class="wpaicg-alert">
    ...
    <?php
    if(empty($wpaicg_cron_sheets_added)){
        echo '<p style="color: #f00"><strong>'.esc_html__('It appears that you have not activated Cron Job on your server, which means you will not be able to use the Google Sheets feature. If you have already activated Cron Job, please allow a few minutes to pass before refreshing the page.','gpt3-ai-content-generator').'</strong></p>';
    }
    else{
        echo '<p style="color: #10922c"><strong>'.esc_html__('Great! It looks like your Cron Job is running properly. You should now be able to use the Google Sheets.','gpt3-ai-content-generator').'</strong></p>';
    }
    ?>
    <?php
    if(!empty($wpaicg_crojob_sheets_last_time)):
        ...
        ?>
        <p><?php echo sprintf(esc_html__('The last time, the Cron Job ran on your website %s (%s ago)','gpt3-ai-content-generator'),date('Y-m-d H:i:s',$wpaicg_crojob_sheets_last_time),$wpaicg_output)?></p>
    <?php
    endif;
    ?>
    <?php
    ...
    <p><code>* * * * * php <?php echo esc_html(ABSPATH)?>index.php -- wpaicg_sheets=yes</code></p>
    <p><?php echo esc_html__('Please note * * * * * means this cron job will run every minute. You can change it to 0 */2 * * * to run every 2 hours. You can also change it to 0 0 * * * to run every day.','gpt3-ai-content-generator')?></p>
    <p><?php echo sprintf(esc_html__('Once you setup the cronjob %syou need to wait for 1 minute%s to see the results.','gpt3-ai-content-generator'),'<b>','</b>')?></p>
    <?php
    //    endif;
    ?>
</div>
